...
function DefaultAvatar({ name, id }) {
  return (
    <div className={`w-11 h-11 rounded-full bg-gradient-to-br ${getGradient(id)} flex items-center justify-center border-2 border-white/10 flex-shrink-0`}>
  ...
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((star) => (
        <svg
          key={star}
          className={`w-4 h-4 ${star <= rating ? 'text-amber-400' : 'text-neutral-700'}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
...
function TestimonialCard({ testimonial }) {
  ...
  return (
    <div className="group relative bg-[#0a0a0a]/60 backdrop-blur-sm border border-white/[0.06] rounded-2xl p-6 transition-all duration-500 hover:border-blue-500/20 hover:shadow-xl hover:shadow-blue-500/5 h-full">
      {/* Quote Icon */}
      <div className="mb-4">
      ...
      {/* Content */}
      <p className="mt-4 text-sm text-neutral-400 leading-relaxed line-clamp-4 group-hover:line-clamp-none transition-all duration-300">
        "{testimonial.content}"
      </p>
      ...
      <div className="mt-6 pt-5 border-t border-white/[0.06]">
        <div className="flex items-center gap-3">
          ...
          {hasAvatar ? (
            <div className="relative flex-shrink-0">
              <div className="w-11 h-11 rounded-full overflow-hidden border-2 border-white/10">
                ...
                />
                {/* Hidden fallback that shows on img error */}
                <div
                  className={`w-full h-full rounded-full bg-gradient-to-br ${getGradient(testimonial.id)} items-center justify-center border-2 border-white/10 hidden`}
                >
                  <span className="text-sm font-bold text-white">{getInitials(testimonial.name)}</span>
                </div>
              </div>
              <span className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-emerald-500 rounded-full border-2 border-[#0a0a0a]" />
            </div>
          ) : (
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-neutral-500 truncate">{testimonial.role}</p>
          </div>
      ...
      {/* Hover accent corner */}
      <div className="absolute top-0 right-0 w-16 h-16 bg-gradient-to-bl from-blue-500/10 to-transparent rounded-bl-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />
    </div>
  );
}
...
function Testimonials() {
  ...
  // Reset page if it exceeds total after data loads
  useEffect(() => {
  ...
  // Adaptive grid class based on items count
  ...
  return (
    <section id="testimonials" className="relative min-h-screen flex items-center overflow-hidden bg-[#050505] pt-24 pb-20">

      {/* ─── Background Effects ─── */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-20 right-20 w-[500px] h-[500px] bg-purple-600/8 rounded-full blur-[150px]" />
        <div className="absolute bottom-40 left-20 w-[400px] h-[400px] bg-blue-600/8 rounded-full blur-[120px]" />
        <div
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 w-full">
        ...
        <div className="flex flex-col items-center text-center mb-16 gap-4">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-white/10 bg-white/[0.03] backdrop-blur-sm">
            ...
            <span className="text-xs font-medium text-neutral-400">Client Reviews</span>
          </div>
        ...
        {loading && (
          <div className="grid md:grid-cols-3 gap-6">
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-[#0a0a0a]/60 border border-white/[0.06] rounded-2xl p-6 animate-pulse">
                <div className="w-8 h-8 bg-white/5 rounded-lg mb-4" />
                <div className="flex gap-1 mb-4">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <div key={s} className="w-4 h-4 bg-white/5 rounded-full" />
                  ))}
                </div>
                ...
                <div className="border-t border-white/[0.06] pt-5 flex items-center gap-3">
                  <div className="w-11 h-11 bg-white/5 rounded-full" />
        ...
        {!loading && testimonials.length > 0 && (
          <div className="relative">
            ...
            <div className="overflow-hidden">
              <div
                className={`grid ${gridClass} gap-6 transition-all duration-300 ease-out ${
                  isSliding
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-4 mt-10">
                <button
                  onClick={prevPage}
                  disabled={isSliding}
                  className="group flex items-center gap-2 px-5 py-2.5 rounded-xl border border-white/10 bg-white/[0.03] text-neutral-400 hover:text-white hover:border-white/20 hover:bg-white/[0.06] disabled:opacity-30 disabled:cursor-not-allowed transition-all duration-300"
                >
            ...
            {/* Page Counter (only if more than 1 page) */}
            {totalPages > 1 && (
        ...
        {!loading && (
          <div className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { value: '50+', label: 'Happy Clients', icon: 'solar:users-group-rounded-bold-duotone', color: 'text-blue-400', bg: 'bg-blue-500/10' },
              { value: '4.9/5', label: 'Avg. Rating', icon: 'solar:star-bold-duotone', color: 'text-amber-400', bg: 'bg-amber-500/10' },
              { value: '120+', label: 'Projects Done', icon: 'solar:folder-bold-duotone', color: 'text-purple-400', bg: 'bg-purple-500/10' },
              { value: '98%', label: 'Satisfaction', icon: 'solar:heart-bold-duotone', color: 'text-pink-400', bg: 'bg-pink-500/10' },
            ].map((stat, i) => (
              <div
                key={i}
                className="group flex items-center gap-4 p-5 rounded-2xl bg-[#0a0a0a]/40 border border-white/[0.06] hover:border-white/10 hover:bg-[#0a0a0a]/60 transition-all duration-300"
              >
                <div className={`w-12 h-12 rounded-xl ${stat.bg} flex items-center justify-center flex-shrink-0`}>
                  <span className={`iconify w-6 h-6 ${stat.color}`} data-icon={stat.icon} />
                </div>
                <div>
        ...
        {!loading && (
          <div className="flex flex-col items-center gap-4 mt-14">
            <p className="text-neutral-500 text-sm">Ready to join our success stories?</p>
            <a
              href="#contact"
              className="group relative px-8 py-3.5 text-sm font-medium text-white rounded-xl overflow-hidden transition-all duration-500 hover:shadow-xl hover:shadow-purple-500/20"
            >
